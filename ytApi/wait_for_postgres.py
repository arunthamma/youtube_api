...
def pg_isready(host, user, password, dbname):
    while time() - start_time < check_timeout:
        try:
            ...
            logger.info("Postgres isn't ready. Waiting for %s %s...", check_interval, interval_unit )
            sleep(check_interval)

    logger.error("We could not connect to Postgres within %s seconds.", check_timeout)
    return False
